        ...
        <div className="flex flex-wrap justify-between items-center mx-auto max-w-7xl">
          ...
          <div className="hidden lg:flex items-center lg:order-2 gap-3">
            {loading ? (
              ...
              <>
                <a href="/profile" className="flex items-center gap-3 px-4 py-2 bg-white/20 rounded-lg border border-white/30 backdrop-blur-sm hover:bg-white/30 transition-all duration-200" title="Lihat Profil">
          ...
          <div className="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
            <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
              ...
              {!loading && isAuthenticated && (
                <>
                  <li><a href="/genres" className="block py-2 pr-4 pl-3 text-white! font-semibold hover:text-white! lg:hover:bg-transparent lg:p-0 rounded transition-all">Genre</a></li>
                  <li><a href="/authors" className="block py-2 pr-4 pl-3 text-white! font-semibold hover:text-white! lg:hover:bg-transparent lg:p-0 rounded transition-all">Penulis</a></li>
                  <li><a href="/profile" className="block py-2 pr-4 pl-3 text-white! font-semibold hover:text-white! lg:hover:bg-transparent lg:p-0 rounded transition-all">Profil</a></li>
                  {user?.role === 'admin' && (
                    <li><a href="/admin" className="block py-2 pr-4 pl-3 text-white! font-semibold hover:text-white! lg:hover:bg-transparent lg:p-0 rounded transition-all">Admin Panel</a></li>
                  )}
                </>
              )}
              <li className="lg:hidden border-t border-white/20 pt-3 mt-3">
                {loading ? (
                  ...
                  <div className="py-2 pl-3">
                    <a href="/profile" className="bg-white/10 rounded-lg px-3 py-2 mb-3 block hover:bg-white/20 transition-all">
                      <p className="text-white font-semibold text-sm mb-1">{user?.name || "User"}</p>
                      <p className="text-white/70 text-xs">{user?.email || ""}</p>
                      <p className="text-white/60 text-xs mt-1 capitalize">Role: {user?.role || "user"}</p>
                    </a>
